...
describe('E2E: CLI Commands', () => {
  ...
  describe('init command', () => {
    ...
    it('debería crear la estructura .context con --yes', () => {
      ...
      assert.ok(
        existsSync(join(E2E_TEST_FOLDER, '.context', 'rules')),
        '.context/rules debería existir'
      );
      assert.ok(
        existsSync(join(E2E_TEST_FOLDER, '.context', 'skills')),
        '.context/skills debería existir'
      );
      assert.ok(
        existsSync(join(E2E_TEST_FOLDER, '.context', 'docs')),
        '.context/docs debería existir'
      );
      assert.ok(
        existsSync(join(E2E_TEST_FOLDER, '.context', 'mcp')),
        '.context/mcp debería existir'
      );
    });
    ...
    it('debería crear archivos de template', () => {
      ...
      assert.ok(
        existsSync(join(E2E_TEST_FOLDER, '.context', 'architecture.md')),
        'architecture.md debería existir'
      );
    });
    ...
    it('debería NO crear archivos puente en modo --yes (quick start)', () => {
      ...
      assert.ok(
        !existsSync(join(E2E_TEST_FOLDER, '.cursorrules')),
        '.cursorrules NO debería existir en quick start'
      );
      assert.ok(
        !existsSync(join(E2E_TEST_FOLDER, '.antigravityrules')),
        '.antigravityrules NO debería existir'
      );
    ...
    it('debería generar AI_BOOTSTRAP.md en modo quick start (--yes)', () => {
      ...
      assert.ok(
        existsSync(join(E2E_TEST_FOLDER, 'AI_BOOTSTRAP.md')),
        'AI_BOOTSTRAP.md debería existir en quick start'
      );
    });
    ...
    it('debería simular sin crear archivos con --dry-run', () => {
      ...
      assert.ok(
        !existsSync(join(E2E_TEST_FOLDER, '.context')),
        'NO debería crear .context en dry-run'
      );
      assert.ok(
        result.output.includes('DRY-RUN') || result.output.includes('simulación'),
        'Debería mencionar dry-run'
      );
    });
    ...
    it('debería respetar archivos existentes sin --force', () => {
      ...
      assert.equal(
        content,
        '# Original Content',
        'El contenido original no debería cambiar sin --force'
      );
    });
  ...
  describe('doctor command', () => {
    ...
    it('debería reparar problemas con --fix', () => {
      ...
      assert.ok(
        existsSync(join(E2E_TEST_FOLDER, '.context', 'rules')),
        'rules debería crearse con --fix'
      );
      assert.ok(
        existsSync(join(E2E_TEST_FOLDER, '.context', 'skills')),
        'skills debería crearse con --fix'
      );
    });
  });
  ...
  describe('update command', () => {
    ...
    it('debería actualizar archivos puente existentes con --yes', () => {
      // Primero crear un bridge manualmente (simular que existía)
      mkdirSync(join(E2E_TEST_FOLDER, '.cursor', 'rules'), { recursive: true });
      writeFileSync(join(E2E_TEST_FOLDER, '.cursor', 'rules', 'global.md'), '# Old content');

      // Ejecutar update con --yes actualiza los existentes
      runCLI('update . --yes');

      // Verificar que se regeneró el bridge
      const content = readFileSync(join(E2E_TEST_FOLDER, '.cursor', 'rules', 'global.md'), 'utf-8');
      assert.ok(
        content.includes('.context') || content.includes('AGENTS.md'),
        'Cursor bridge debería tener contenido actualizado'
      );
      assert.notEqual(content, '# Old content', 'El contenido debería cambiar');
    });

    it('debería verificar .agent-ctx.json existe', () => {
      // Verificar que init creó el config
      assert.ok(
        existsSync(join(E2E_TEST_FOLDER, '.agent-ctx.json')),
        '.agent-ctx.json debería existir después de init'
      );

      const config = JSON.parse(readFileSync(join(E2E_TEST_FOLDER, '.agent-ctx.json'), 'utf-8'));
      assert.ok(Array.isArray(config.skills), 'config debería tener skills array');
      assert.ok(config.skills.length > 0, 'skills debería tener elementos');
    });

    it('debería reportar si no hay config', () => {
      // Limpiar y no inicializar
      cleanTestFolder();

      const result = runCLI('update . --yes');

      // Debería fallar o advertir sobre falta de config
      assert.ok(
        !result.success || result.output.includes('.agent-ctx.json'),
        'Debería indicar que falta .agent-ctx.json'
      );
    });
  });
  ...
  describe('clean command', () => {
    ...
    it('debería detectar carpetas de editores redundantes', () => {
      ...
      assert.ok(
        result.output.includes('.cursor') || result.output.includes('redundante'),
  ...
  describe('centralize command', () => {
    ...
    it('debería detectar skills dispersas en carpetas de editores', () => {
      ...
      writeFileSync(
        join(E2E_TEST_FOLDER, '.cursor', 'skills', 'my-skill', 'SKILL.md'),
        '# My Skill'
      );
      ...
      assert.ok(
        result.output.includes('.cursor') ||
          result.output.includes('skill') ||
          result.output.includes('Movería'),
    ...
    it('debería limpiar carpetas de editores vacías después de mover', () => {
      ...
      writeFileSync(
        join(E2E_TEST_FOLDER, '.windsurf', 'skills', 'only-skill', 'index.md'),
        '# Only'
      );
      ...
      assert.ok(
        !existsSync(join(E2E_TEST_FOLDER, '.windsurf')),
